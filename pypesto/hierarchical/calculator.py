...
class HierarchicalForwardAmiciCalculator(HierarchicalAmiciCalculator):
    ...
    def __call__(self, obj, x, sensi_order, mode):
        ...
        if self.problem.optimalScalingOptions['timeInnerProblem']:
            ...
            with open('./results/timing_results_' + self.problem.optimalScalingOptions['modelName'] + '_' + str(
                    date.today())
...
def compute_optimal_surrogates_edatas(problem, edatas, rdatas):
    ...
    for gr in problem.get_groups_for_xs(HierarchicalParameter.OPTIMALSCALING):
        ...
        surrogate_opt_results = optimize_surrogate_data(xs, rdatas, problem)
...
def optimize_surrogate_data(xs, rdatas, problem):
    from scipy.optimize import minimize
    parameter_length = 2 * len(xs)

    w = get_weight_for_surrogate2(xs, rdatas)
    ...
    obj_surr = lambda x: obj_surrogate_data(xs, x, rdatas, w)
...
def obj_surrogate_data(xs, optimal_scaling_bounds, rdatas, w):
...
def get_qualitative_data_indices(problem):
...
def get_surrogate_data(xs, optimal_scaling_bounds, rdatas):
    y_sim_all = []
    y_surrogate_all = []
    x_lower_all = []
    x_upper_all = []
    ...
    return sort_results(np.array(y_sim_all), time_idx), sort_results(np.array(y_surrogate_all), time_idx), sort_results(
        np.array(x_lower_all), time_idx), sort_results(np.array(x_upper_all), time_idx)


def get_surrogate_data_reduced(xs, optimal_scaling_bounds, rdatas, problem):
    y_sim_all = []
    y_surrogate_all = []
    x_lower_all = []
    x_upper_all = []
...
def get_constraints_for_optimization(xs, rdatas, problem):
    ...
    interval_range, interval_gap = compute_interval_constraints(xs, rdatas, problem)
    # A*x-b>=0, with A[i,i] == -1, A[i,i+1] == 1, A[i,j] ==0 otherwise,
    # b= [interval_range, interval_gap, interval_range, interval_gap, ...]
    '''
...
def compute_optimal_surrogates_edatas_reduced(problem, edatas, rdatas):
    ...
    for gr in problem.get_groups_for_xs(HierarchicalParameter.OPTIMALSCALING):
        xs = problem.get_xs_for_group(gr)
        surrogate_opt_results = optimize_surrogate_data_reduced(xs, rdatas, problem)
        write_surrogate_to_edatas_reduced(surrogate_opt_results, xs, edatas, rdatas, problem)
        optimal_surrogates.append(surrogate_opt_results)
    return optimal_surrogates


def optimize_surrogate_data_reduced(xs, rdatas, problem):
    from scipy.optimize import minimize
    from scipy.optimize import Bounds
    ...
    if problem.optimalScalingOptions['reparameterized']:
        obj_surr = lambda x: obj_surrogate_data_reduced_reparameterized(xs, x, rdatas, interval_gap,
                                                                        interval_range, w)
        if problem.optimalScalingOptions['multistart']:
            results_all = []
            num_starts = 100
            ...
            x0 = y2xi(np.linspace(np.max([min_all, interval_range]), max_all + interval_range, parameter_length), xs,
        ...
        obj_surr = lambda x: obj_surrogate_data_reduced(xs, x, rdatas, interval_gap, w)
        if problem.optimalScalingOptions['multistart']:
            results_all = []
            num_starts = 100
            for startIdx in range(num_starts):
            ...
            results = minimize(obj_surr, x0=np.linspace(np.max([min_all, interval_range]), max_all + interval_range,
                                                        parameter_length),
                               method='SLSQP', constraints=constraints,
                               options={'maxiter': 2000, 'ftol': 1e-10})
...
def obj_surrogate_data_reduced(xs, optimal_scaling_bounds, rdatas, interval_gap, w):
    # compute optimal scaling objective function
    obj = 0.0
    y_sim_all = []
    y_surrogate_all = []
...
def obj_surrogate_data_reduced_reparameterized(xs, optimal_scaling_bounds_reparameterized, rdatas, interval_gap,
                                               interval_range, w):
    # compute optimal scaling objective function
    obj = 0.0
    y_sim_all = []
...
def xi2y(optimal_scaling_bounds_reparameterized, xs, interval_gap, interval_range):
    ...
    for x in xs:
        x_category = int(x.category)
        if x_category == 1:
            optimal_scaling_bounds[x_category - 1] = interval_range + optimal_scaling_bounds_reparameterized[
                x_category - 1]
        else:
...
def get_weight_for_surrogate2(xs, rdatas):
    y_sim_all = []
    for x in xs:
        for condition_ix, time_ix, observable_ix in x.iterate():
            y_sim = rdatas[condition_ix]['y'][time_ix, observable_ix]
...
def compute_wls(optimal_surrogate, problem, edatas, rdatas):
    qualitative_data_indices = get_qualitative_data_indices(problem)
